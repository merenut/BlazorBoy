...
Status: Initial architecture & execution roadmap.
...
Harness Logic:
- Load ROM → run frames → check memory region or serial buffer for "PASS"
- Skip tests if ROM file missing (environment-based)
...
1. Expand Mmu with echo handling & register placeholders.
2. Implement opcode table skeleton (NOP, LD r,r, some immediate loads).
3. Add InterruptController IF/IE layout & wire to Mmu.
4. Introduce unit test project (GameBoy.Tests) with initial memory tests.
5. Replace Ppu placeholder loop with mode cycle skeleton (no pixels yet).
6. Integrate a simple RunFrame loop in Emulator:
   - Accumulate cycles until ~70224 reached.
7. Prepare harness for a single Blargg ROM (manual run).
...
This document provides a stepwise plan to evolve the current scaffold (basic CPU, PPU placeholder, minimal ROM loading) into a fully functional Game Boy emulator in Blazor. Follow the phase ordering, validate with test ROMs incrementally, and defer complexity (APU + advanced timing) until core correctness is locked.

---

Need follow-up artifacts (e.g., opcode table stub, test harness template)? Ask and I can generate file stubs next.
